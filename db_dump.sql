...
--
-- Name: calculate_broker_fee(numeric); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.calculate_broker_fee(price numeric) RETURNS numeric
...
ALTER FUNCTION public.calculate_broker_fee(price numeric) OWNER TO postgres;
...
BEGIN
    ...
    SELECT calculate_broker_fee(p_price) INTO v_fee;
...
DECLARE
    ...
    sell_price                   NUMERIC;
    sell_fee_per_unit            NUMERIC;

    -- Transaction computation
    transaction_quantity         INT;
    executed_fee                 NUMERIC;

    -- Portfolio update vars
    old_qty                      INT;
    old_avg                      NUMERIC;
BEGIN
    -- Fetch buy order data (including price and fee)
    SELECT customer_account_id, security_id, savings_account_id,
           available_quantity, price, fee
    INTO buyer_id, buy_security_id, buyer_savings_account_id,
        available_quantity_buy, buy_price, buy_fee_per_unit
    FROM orders
    WHERE id = buy_order_id;

    -- Fetch sell order data (including price and fee)
    SELECT customer_account_id, security_id, savings_account_id,
           available_quantity, price, fee
    INTO seller_id, sell_security_id, seller_savings_account_id,
        available_quantity_sell, sell_price, sell_fee_per_unit
    FROM orders
    WHERE id = sell_order_id;

    -- Verify security match
    ...
    -- Compute executed fee via broker logic (for logging)
    executed_fee := calculate_broker_fee(executed_price) * transaction_quantity;

    -- Update seller portfolio and account balance
    UPDATE customer_portfolios
    SET reserved_quantity = reserved_quantity - transaction_quantity,
        ...
        avg_sell_price    = (
                                COALESCE(avg_sell_price * sold_quantity, 0)
                                    + executed_price * transaction_quantity
                                    - sell_fee_per_unit * transaction_quantity
                                ) / (COALESCE(sold_quantity, 0) + transaction_quantity)
    WHERE customer_account_id = seller_id
      AND security_id         = sell_security_id;

    UPDATE savings_accounts
    SET balance = balance + (executed_price * transaction_quantity - sell_fee_per_unit * transaction_quantity)
    WHERE id = seller_savings_account_id;

    -- Update buyer account balance and reserved amount using order price and fee
    UPDATE savings_accounts
    SET balance         = balance - (buy_price * transaction_quantity + buy_fee_per_unit * transaction_quantity),
        reserved_amount = reserved_amount - (buy_price * transaction_quantity + buy_fee_per_unit * transaction_quantity)
    WHERE id = buyer_savings_account_id;

    -- Insert or update buyer portfolio using order price and fee
    SELECT total_quantity, avg_buy_price
    INTO old_qty, old_avg
    FROM customer_portfolios
    WHERE customer_account_id = buyer_id
      AND security_id          = buy_security_id
        FOR UPDATE;
    ...
    IF FOUND THEN
        ...
        SET total_quantity = old_qty + transaction_quantity,
            avg_buy_price  = (
                                 old_avg * old_qty
                                     + buy_price * transaction_quantity
                                     + buy_fee_per_unit * transaction_quantity
    ...
    ELSE
        INSERT INTO customer_portfolios (
            ...
            avg_buy_price
                     ...
                     0,
                     (buy_price + buy_fee_per_unit)
                 );
    END IF;

    -- Update buy order status and remaining quantity
    UPDATE orders
    SET available_quantity = CASE
                                 WHEN available_quantity - transaction_quantity > 0 THEN available_quantity - transaction_quantity
                                 ELSE NULL
        END,
        order_status_id    = CASE
                                 WHEN available_quantity - transaction_quantity > 0 THEN 3  -- Partially filled
                                 ELSE 2  -- Filled
            END
    WHERE id = buy_order_id;

    -- Update sell order status and remaining quantity
    UPDATE orders
    SET available_quantity = CASE
                                 WHEN available_quantity - transaction_quantity > 0 THEN available_quantity - transaction_quantity
                                 ELSE NULL
        END,
        order_status_id    = CASE
                                 WHEN available_quantity - transaction_quantity > 0 THEN 3
                                 ELSE 2
            END
    WHERE id = sell_order_id;
...
21	1	2	1	438.67	1.31601	1	\N	2025-05-06 12:11:08.486781	2	2
...
461	1	24	1	103.09	0.30927	1	\N	2025-05-06 12:11:08.486781	2	2
...
542	2	28	2	46.12	0.41508	3	2	2025-05-06 12:11:08.486781	2	3
...
341	1	18	1	74.20	0.22260	1	\N	2025-05-06 12:11:08.486781	2	2
\.
...
1	1	865447751279612180033026976	1	42354.61	0.00000
...
2	2	13827802111912004142304	1	29162.85	0.00000
\.
...
SELECT pg_catalog.setval('public.balance_history_id_seq', 38, true);
...
SELECT pg_catalog.setval('public.customer_accounts_id_seq', 44, true);
...
SELECT pg_catalog.setval('public.orders_id_seq', 611, true);
...
SELECT pg_catalog.setval('public.savings_accounts_id_seq', 39, true);
...
--
-- Name: customer_accounts uq_customer_accounts_login; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.customer_accounts
    ADD CONSTRAINT uq_customer_accounts_login UNIQUE (login);
...
--
-- Name: PROCEDURE deposit_balance(IN p_amount numeric, IN p_currency_id integer, IN p_customer_account_id integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON PROCEDURE public.deposit_balance(IN p_amount numeric, IN p_currency_id integer, IN p_customer_account_id integer) TO app_developer;


--
-- Name: PROCEDURE withdraw_balance(IN p_amount numeric, IN p_currency_id integer, IN p_customer_account_id integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON PROCEDURE public.withdraw_balance(IN p_amount numeric, IN p_currency_id integer, IN p_customer_account_id integer) TO app_developer;
...
GRANT ALL ON SEQUENCE public.orders_id_seq TO app_developer;
